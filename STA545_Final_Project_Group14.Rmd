...
We fit the principal component regression model with all possible values of components on the training dataset ; in order to find  the number of components that maximize the value of R^2.
We found that if we have a number of components egals to 7 which means we keep all our components, we have an optimal r-squared value.
For the PCR regression model, the r-squared value is around 0.25 which means that the model describes around 25% of the variance.
...
r2
```
We fit the ridge model on the training data with an optimally tuned ridge shrinkage parameter λ. 
We also used the rstudio function cv.glmet which use cross validation to perform the tuning. 
The R\^2 value is 0.22, meaning 22.0% of the error is explained by the model.
...
coef(lassofit)
```
The lasso model does not use the variable hour, weekday and windspeed.
the variable atemp has the highest coefficient value.
...
7AM to 9AM and 4PM to 7PM is rush hour, coded as 3.
10 AM to 3PM is midday, coded as 2.
5 AM to 6 AM and 8PM to 11PM is odd hours, coded as 1. 
12AM to 4 AM is night, coded as 0.
